...
# Press Enter twice when prompted for a passphrase (leave empty for passwordless access)
```
...
6. Set the right permissions:
   ```
   chmod -v 700 ~/.ssh && chmod -v 600 ~/.ssh/* && chmod -v 644 ~/.ssh/*.pub 2>/dev/null; true
   ```
