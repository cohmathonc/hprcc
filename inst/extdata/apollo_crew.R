...
default <- crew.cluster::crew_controller_slurm(
    name = "default",
    slurm_memory_gigabytes_per_cpu = 2L,
    slurm_cpus_per_task = 1,
    slurm_time_minutes = 120L,
    host = Sys.info()["nodename"],
    workers = 350L,
    seconds_idle = 15,
    script_directory = here::here("tmp"),
    slurm_log_output = if (isTRUE(log_slurm)) here::here(glue::glue("tmp/slurm-%j.out")) else NULL,
    slurm_log_error = if (isTRUE(log_slurm)) here::here(glue::glue("tmp/slurm-%j.err")) else NULL,
...
huge <- crew.cluster::crew_controller_slurm(
    name = "huge",
    slurm_cpus_per_task = 40L,
    slurm_time_minutes = 720L,
    host = Sys.info()["nodename"],
    workers = 350L,
    seconds_idle = 30,
...
tar_option_set(
    ...
    retrieval = "worker", #
    controller = crew::crew_controller_group(local, small, medium, large, bigmem, huge),
...
slurm_huge <- tar_resources(
      crew = tar_resources_crew(controller = "huge")
    )
